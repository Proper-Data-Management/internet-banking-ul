...
func Accurate(searchText string) string {
	return fmt.Sprintf("%s%%", searchText)
}

func Inaccurate(searchText string) string {
	return fmt.Sprintf("%%%s%%", searchText)
}
...
// OrLikeByColumns is syntactic sugar for use with LIKE conditions.
// Ex:
// .Where(OrLikeByColumns{"%irrel", []string{"name","external_id"}})
type OrLikeByColumns map[string]interface{}

func (lk OrLikeByColumns) toSql(opr string) (sql string, args []interface{}, err error) {
	var exprs []string
		...
		if val == nil {
			...
			if isListType(val) {
				...
				if valVal.Len() == 0 {
					...
					for i := 0; i < valVal.Len(); i++ {
						expr = fmt.Sprintf("%s %s ?", valVal.Index(i).Interface(), opr)
						args = append(args, key)
						exprs = append(exprs, expr)
					}
				}
			} else {
				expr = fmt.Sprintf("%s %s ?", val, opr)
				args = append(args, key)
				exprs = append(exprs, expr)
			}
		}
...
func (lk OrLikeByColumns) ToSql() (sql string, args []interface{}, err error) {
	return lk.toSql("LIKE")
}

// OrLikeByValues is syntactic sugar for use with LIKE conditions.
// Ex:
// .Where(OrLikeByValues{"name", []string{"%irrel","%test"}})
type OrLikeByValues map[string]interface{}

func (lk OrLikeByValues) toSql(opr string) (sql string, args []interface{}, err error) {
	var exprs []string
	for key, val := range lk {
		expr := ""
...
func (lk OrLikeByValues) ToSql() (sql string, args []interface{}, err error) {
	return lk.toSql("LIKE")
}
