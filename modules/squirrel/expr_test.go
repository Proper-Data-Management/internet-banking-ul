...
func TestOrLikeByColumnsToSql(t *testing.T) {
	b := OrLikeByColumns{"%irrel": []string{"name", "test"}}
	sql, args, err := b.ToSql()
	assert.NoError(t, err)

	expectedSql := "name LIKE ? OR test LIKE ?"
	assert.Equal(t, expectedSql, sql)

	expectedArgs := []interface{}{"%irrel", "%irrel"}
	assert.Equal(t, expectedArgs, args)
}

func TestOrLikeByValuesToSql(t *testing.T) {
	b := OrLikeByValues{"name": []string{"%irrel", "%test"}}
	sql, args, err := b.ToSql()
	assert.NoError(t, err)

	expectedSql := "name LIKE ? OR name LIKE ?"
	assert.Equal(t, expectedSql, sql)

	expectedArgs := []interface{}{"%irrel", "%test"}
	assert.Equal(t, expectedArgs, args)
}
